...
def Read_in_DataFrame(file_path_pr): #Read in DataFrame, DF1, containing extracted peaks
    loc = file_path_pr + "/Clustered_Data.xlsx" #Locate the desired file
...
def Group_Assignment(mmdf, mmwdf, filenames, PP_len_label, Position_Match, Width_Match, writer):
    ...
    for fn, f in enumerate(filenames): #For each filename
        ...
        for ln, p in enumerate(mmdf['together']): #For each Clustered Postions row
            if f[:PP_len_label] in p: #If filename is within row
                final.loc[lenf,  'Filename'] = f #Add filename
                final.loc[lenf,  'File_name'] = f[:PP_len_label] #Add reduced filename
        ...
        for ln, w in enumerate(mmwdf['together']): #For each Clustered Widths row
            if f[:PP_len_label] in w: #If filename is within row
                final.loc[lenf,  'Filename'] = f #Add filename
                final.loc[lenf,  'File_name'] = f[:PP_len_label] #Add reduced filename
...
def Generate_Clustered_Folders(filenames, file_path_pr, file_path_c, PP_len_label, writer, final, result, Visualisations_PrP, Visualisations_PrW): #Generate clustered folders
    c_file_path = os.getcwd() #Get current directory
    if Visualisations_PrP == 'Yes':
        for p in result.columns[result.columns.str.startswith('P')]: #For columns starting with 'P'
            for name, i in zip(result['Filename'], result[str(p)]): #For each filename and assigned group
                if str(i) != 'None': #If cell is not 'None'
                    ...
                    shutil.copy(file_path_pr + '/' + str(name) , new_folder + '/' + str(name)) #Copy original data from original file path to new folderto new folder
                    os.chdir(file_path_c) #Change directory to desired directory
    if Visualisations_PrW == 'Yes':
...
def Visualise_Folder(file_path, file_path_p, file_extension, PP_len_label): #Visualise the data in each folder
...
def Visualisation_Loop(file_path_c, file_path_p, file_extension, PP_len_label, Visualisations_PrP, Visualisations_PrW): #Control loop for data visualisation
    if Visualisations_PrP == 'Yes':
        file_path = file_path_c + '/Positions' #Change path to specified directory
        Visualise_Folder(file_path, file_path_p, file_extension, PP_len_label)  #Visulaise the data in each folder
    if Visualisations_PrW == 'Yes':
        file_path = file_path_c + '/Widths' #Change path to specified directory
        Visualise_Folder(file_path,  file_path_p, file_extension, PP_len_label)  #Visulaise the data in each folder
    return 

def Pr_Clustering_Loop_3(file_path_pr, file_extension, PP_len_label, Position_Match, Width_Match, Visualisations_PrP, Visualisations_PrW): #Control Loop
    ...
    writer.book = load_workbook(file_path_pr + '/Clustered_Data.xlsx')
    ...
    final, result = Group_Assignment(mmdf, mmwdf, filenames, PP_len_label, Position_Match, Width_Match, writer)
    Generate_Clustered_Folders(filenames, file_path_pr, file_path_c, PP_len_label, writer, final, result, Visualisations_PrP, Visualisations_PrW) #Generate clustered folders
    Visualisation_Loop(file_path_c, file_path_p, file_extension, PP_len_label, Visualisations_PrP, Visualisations_PrW)  #Control loop for data visualisation
    print('Finished')
    return