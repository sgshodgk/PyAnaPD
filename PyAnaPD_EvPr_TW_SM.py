...
def Visualise_shifts(file_path_c, file_path_ppp, filenames, filenames_pr, EP_len_label, PP_len_label, spm, Max_d, lamda1, lamda2, step_size_thres, newpath): #Visualise peak shifts
    pi = math.pi
    ...
    for ln, (cp, pp) in enumerate(zip(spm['EP'], spm['PP'])): #For matched experimental and predicted patterns (match >= EvP_Match_Pr)
        ...
        two_theta, intensity, dspace = Read_Data(file_name, lamda1, lamda2, step_size_thres) #Read in picked peaks file
        mask = dspace < Max_d #Indentify d space below Max_d (mask)
        cpt_d = dspace[mask]
        ...
        two_theta, intensity, dspace = Read_Data(file_name, lamda1, lamda2, step_size_thres) #Read in picked peaks file
        mask = dspace < Max_d #Indentify d space below Max_d (mask)
        ppt_d = dspace[mask] #Cut d space above Max_d
        pr_intensity = intensity[mask] #Cut intensities above mask
        
        smm = MF1[(MF1['EP'] == str(cp)) & (MF1['PP'] == str(pp))] #Identify matched data corresponding to experimental and predicted pattern
        sm = smm.iloc[:, 3:].copy()
        sm = sm[sm < Max_d]
...
def Read_Data(file_name, lamda1, lamda2, step_size_thres): #Read in data
    pi = math.pi
    ...
    ss = two_theta[1]-two_theta[0]
    if ss >= step_size_thres:
        dspace = lamda1/(2*np.sin((two_theta/2)*pi/180)) #Convert experimental to d space
    if ss < step_size_thres:
        dspace = lamda2/(2*np.sin((two_theta/2)*pi/180)) #Convert experimental to d space
    np.seterr(divide='ignore', invalid='ignore') #Ignore floating point errors
    return two_theta, intensity, dspace

def TW_ExperimentalvPredicted_Selective_Matching(file_path, file_path_pr, file_extension,  EP_len_label, PP_len_label, lamda1, lamda2, step_size_thres, Intensity_difference_tol, I_Match, EvP_Match_Pr, EvP_Match_Ex, Max_d, Visualisations_SM): #Control Loop
    ...
    if Visualisations_SM == 'Yes':
        Visualise_shifts(file_path_c, file_path_ppp, filenames, filenames_pr, EP_len_label, PP_len_label, spm, Max_d, lamda1, lamda2, step_size_thres, newpath) #Visualise peak shifts
    print('Finished')
    return