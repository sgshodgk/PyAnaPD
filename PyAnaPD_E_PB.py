...
def Porosity(Peak_below, file_path_c, file_path, DF1, filenames): #Determine files have a peak below the specified two theta value given
...
def main(Peak_below, file_path_c, file_path, filenames): #Main loop to control functions
    DF1 = Read_in_DataFrame(file_path_c) #Read in DataFrame, DF1, containing extracted peaks
    Porosity(Peak_below, file_path_c, file_path, DF1, filenames) #Determine files have a peak below the specified two theta value given
    return 
...
def Visualisation_Loop(file_path_c, file_path_br, file_extension, EP_len_label, Peak_below): #Control loop
...
def Peak_Below_Loop(file_path, file_extension, EP_len_label,  Peak_below): #Control loop
    ...
    for ex in file_extension: #For each given file extension
        filenames.extend(natsorted(glob.glob(ex))) #Identify files with set file extension
    main(Peak_below, file_path_c, file_path, filenames) #Send data to main loop
    Visualisation_Loop(file_path_c, file_path_br, file_extension, EP_len_label, Peak_below) #Control loop
    print('Finished')
    return