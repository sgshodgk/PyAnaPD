...
def Read_in_DataFrame(file_path_c, file_path_ppp, lamda1, lamda2, step_size_thres): #Read in DataFrames containing extracted peaks (experimental and predicted)
    ...
    w = DF1[DF1['step_size'] >= step_size_thres].copy()
    cd_s1 = lamda1/(2*np.sin((w['two_theta']/2)*pi/180)) #Convert experimental to d space
    w.loc[:,'d'] = cd_s1
    x = DF1[DF1['step_size'] < step_size_thres].copy()
    cd_s2 = lamda2/(2*np.sin((x['two_theta']/2)*pi/180)) #Convert experimental to d space
    x.loc[:,'d'] = cd_s2
    DF1 = pd.concat([w, x]).reset_index()

    y = P_DF1[P_DF1['step_size'] >= step_size_thres].copy()
    cd_s1 = lamda1/(2*np.sin((y['two_theta']/2)*pi/180)) #Convert experimental to d space
    y.loc[:,'d'] = cd_s1
    z = P_DF1[P_DF1['step_size'] < step_size_thres].copy()
    cd_s2 = lamda2/(2*np.sin((z['two_theta']/2)*pi/180)) #Convert experimental to d space
    z.loc[:,'d'] = cd_s2
    P_DF1 = pd.concat([y, z]).reset_index()
    return DF1, P_DF1
...
def Similarities(DF1, P_DF1, filenames, filenames_pr, EP_len_label, PP_len_label, Sig_peaks, Inital_difference_tol, Difference_tol, writer): #Identifying similarities between experimental and predicted patterns
    ...
    for linei, i in enumerate(filenames): #For each experimental pattern
        for linej, j in enumerate(filenames_pr): #For each predicted pattern
            ...
            #if int((len(pdf)/len(e[j])*100)) >= EvP_Match_Pr and int((len(pdf)/len(d[i])*100)) >= EvP_Match_Ex: #If experimental pattern contains atleast EvP_Match_Pr (eg 70%) of predicted peaks
...
def TW_ExperimentalvPredicted_Similarities(file_path, file_path_pr, file_extension, EP_len_label, PP_len_label, lamda1, lamda2, step_size_thres, Sig_peaks, Inital_difference_tol, Difference_tol): #Control Loop
    ...
    DF1, P_DF1 = Read_in_DataFrame(file_path_c, file_path_ppp, lamda1, lamda2, step_size_thres) #Read in DataFrame, DF1, containing extracted peaks
