...
def Peak_in_range(peak_minimum, peak_maximum, file_path_c, file_path_pr, DF1, filenames): #Determine files have a peak between specified two theta values given
...
def main(peak_minimum_two_theta, peak_maximum_two_theta, file_path_c, file_path_pr, filenames): #Main loop to control functions
    DF1 = Read_in_DataFrame(file_path_c) #Read in DataFrame, DF1, containing extracted peaks
    Peak_in_range(peak_minimum_two_theta, peak_maximum_two_theta, file_path_c, file_path_pr, DF1, filenames) #Determine files have a peak between specified two theta values given
    return 
...
def Visualisation_Loop(file_path_pr, file_path_c, file_path_p, file_extension, PP_len_label, peak_minimum, peak_maximum): #Control loop
    ...
    if os.path.exists('No Peak between ' + str(peak_minimum) + ' & ' + str(peak_maximum)+ ' 2θ (°)'): #If 'no peak between' folder exists
        ...
        max_tt, min_tt, = twotheta_range(file_path_pr, file_extension, PP_len_label) #Determine two theta ranges of collected patterns
        Visualise_Folder(file_path_pb, file_path_c, file_path_p, file_extension, PP_len_label, max_tt, min_tt, file)  #Visulaise the data in each folder
    return 

def Pr_Peak_in_Range_Loop(file_path_pr, file_extension, PP_len_label, peak_minimum, peak_maximum): #Control loop
    ...
    for ex in file_extension: #For each given file extension
        filenames.extend(natsorted(glob.glob(ex))) #Identify files with set file extension
    main(peak_minimum, peak_maximum, file_path_c, file_path_pr, filenames) #Send data to main loop
    Visualisation_Loop(file_path_pr, file_path_c, file_path_p, file_extension, PP_len_label, peak_minimum, peak_maximum) #Control loop
    print('Finished')
    return