...
def Read_in_DataFrame(file_path_c): #Read in DataFrame, DF1, containing extracted peaks
    loc = file_path_c + "/Fitted_Picked_Peaks.xlsx" #Locate desired excel file
    DF1 = pd.read_excel(loc) #Read in file as a dataframe, DF1
    return DF1

def Similarities(DF1, PP_len_label, Position_tol, Width_tol, writer): #Identify similarities between PXRD patterns
    filenames = DF1['file_name'].unique() #Determine unique filenames in DF1
    mdf = pd.DataFrame(columns=[i[:PP_len_label] for i in filenames]) #Set all filenames as columns
    mdf['files']=[i[:PP_len_label] for i in filenames] #Add a column of filenames
    mdf = mdf.set_index('files') #Set column of filenames as index (rows)
    mwdf = pd.DataFrame(columns=[i[:PP_len_label] for i in filenames]) #Set all filenames as columns
    mwdf['files']=[i[:PP_len_label] for i in filenames] #Add a column of filenames
    ...
    for line, i in enumerate(filenames[:-1]): #For each filename, except the last
        for e in filenames[(line+1):]: #For each succeeding filename
            ...
            i_p = ((len(pdf))/len(d[i]['two_theta'])*100) #Determine % number of matched peaks in file i
            e_p = ((len(pdf))/len(d[e]['two_theta'])*100) #Determine % number of matched peaks in file e
            ...
            mdf.loc[(i[:PP_len_label]), i[:PP_len_label]] = 100 #Set same column/row name as 100% match
            mdf.loc[(e[:PP_len_label]), i[:PP_len_label]] = i_p #Determine % number of matched peaks in file i
            mdf.loc[(i[:PP_len_label]), e[:PP_len_label]] = e_p #Determine % number of matched peaks in file e
            mdf.loc[(e[:PP_len_label]), e[:PP_len_label]] = 100 #Set same column/row name as 100% match
            mwdf.loc[(i[:PP_len_label]), i[:PP_len_label]] = 100 #Set same column/row name as number of widths/peaks
            mwdf.loc[(e[:PP_len_label]), i[:PP_len_label]] = i_w  #Determine % number of matched peaks in file i
            mwdf.loc[(i[:PP_len_label]), e[:PP_len_label]] = e_w #Determine % number of matched peaks in file e (same as i)
            mwdf.loc[(e[:PP_len_label]), e[:PP_len_label]] = 100 #Set same column/row name as number of widths/peaks
...
def Pr_Clustering_Loop_1(file_path_pr, PP_len_label, Position_tol, Width_tol): #Control Loop
    ...
    mdf, wdf = Similarities(DF1, PP_len_label, Position_tol, Width_tol, writer) #Send to function Similarities (Identifying similarities between PXRD patterns)
    print('Finished')
    return 